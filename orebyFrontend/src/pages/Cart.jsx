...
const Cart = () => {
  ...
  useEffect(() => {
    async function getAllcart() {
      axios
        .get(
          "http://localhost:3000/api/v1/cart/getcart/667bf245cf8c85faf332a1ab"
        )
        .then((data) => {
  ...
  let totalprice = cartData.reduce(
    (prev, curr) => curr.productid.sellingprice * curr.quntity + prev,
    0
  );
      ...
      <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-center justify-between lg:px-6 pb-6 border-b border-gray-200 max-lg:max-w-lg max-lg:mx-auto">
          ...
          <div className="flex items-center justify-between gap-5 ">
            <h6 className="font-manrope  text-nowrap font-bold text-3xl lead-10 text-indigo-600">
              {totalprice} Taka
            </h6>
          </div>
        </div>
        <div className="max-lg:max-w-lg max-lg:mx-auto">
          ...
          <Link
            ...
            className="w-full rounded-full py-4 px-6 bg-indigo-600 text-white font-semibold text-lg w-full text-center transition-all duration-500 hover:bg-indigo-700 "
          >
            Checkout
          </Link>
        </div>
